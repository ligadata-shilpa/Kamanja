...
echo "...Command = java -cp $installDir/lib/system/jarfactoryofmodelinstancefactory_2.11-1.4.1.jar:$installDir/lib/system/ExtDependencyLibs2_2.11-1.4.1.jar:$installDir/lib/system/ExtDependencyLibs_2.11-1.4.1.jar:$installDir/lib/system/KamanjaInternalDeps_2.11-1.4.1.jar:$installDir/lib/system/nodeinfoextract_2.11-1.4.1.jar com.ligadata.installer.NodeInfoExtra --MetadataAPIConfig \"$metadataAPIConfig\" --workDir \"$workDir\" --ipFileName \"$ipFile\" --ipPathPairFileName \"$ipPathPairFile\" --ipIdCfgTargPathQuartetFileName \"$ipIdCfgTargPathQuartetFileName\" --installDir \"$installDir\" --clusterId \"$clusterId\""
java -cp $installDir/lib/system/jarfactoryofmodelinstancefactory_2.11-1.4.1.jar:$installDir/lib/system/ExtDependencyLibs2_2.11-1.4.1.jar:$installDir/lib/system/ExtDependencyLibs_2.11-1.4.1.jar:$installDir/lib/system/KamanjaInternalDeps_2.11-1.4.1.jar:$installDir/lib/system/nodeinfoextract_2.11-1.4.1.jar com.ligadata.installer.NodeInfoExtra --MetadataAPIConfig $metadataAPIConfig --workDir "$workDir" --ipFileName "$ipFile" --ipPathPairFileName "$ipPathPairFile" --ipIdCfgTargPathQuartetFileName "$ipIdCfgTargPathQuartetFileName" --installDir "$installDir" --clusterId "$clusterId"
if [ "$?" -ne 0 ]; then
    echo
