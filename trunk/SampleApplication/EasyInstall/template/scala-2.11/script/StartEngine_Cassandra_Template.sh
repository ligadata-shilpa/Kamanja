...
if [ "$1" != "debug" ]; then
	java -Dlog4j.configurationFile=file:{InstallDirectory}/config/log4j2.xml -cp $KAMANJA_HOME/lib/system/ExtDependencyLibs_2.11-1.4.0:$KAMANJA_HOME/lib/system/KamanjaInternalDeps_2.11-1.4.0:{InstallDirectory}/bin/KamanjaManager-1.0 com.ligadata.kamanjamanager --config {InstallDirectory}/config/Engine1Config_Cassandra.properties
else
