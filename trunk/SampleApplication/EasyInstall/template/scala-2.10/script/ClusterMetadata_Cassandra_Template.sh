...
if [ "$1" != "debug" ]; then
	java -Dlog4j.configurationFile=file:{InstallDirectory}/config/log4j.properties -cp $KAMANJA_HOME/lib/system/ExtDependencyLibs_2.10-1.0:$KAMANJA_HOME/lib/system/KamanjaInternalDeps_2.10-1.0:{InstallDirectory}/bin/MetadataAPI-1.0 com.ligadata.metadataapi--config {InstallDirectory}/config/ClusterCfgMetadataAPIConfig_Cassandra.properties
else
