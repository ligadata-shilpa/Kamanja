...
class CassandraAdapter(val kvManagerLoader: KamanjaLoaderInfo, val datastoreConfig: String) extends DataStore {
  ...
  private def dropTable(tableName: String,ks:String = null): Unit = lock.synchronized {
    ...
    try {
      ...
      case e: Exception => {
        throw CreateDDLException("Unable to drop table " + fullTableName + ":" + e.getMessage(), e)
      }
    }
  }
  ...
  private def IsTableExists(tableName: String,ks:String = null): Boolean = {
	...
	keyspaceSearched = ks
      ...
      case e: Exception => {
        throw CreateDMLException("Unable to verify whether table " + keyspaceSearched + "." + tableName + " exists:" + e.getMessage(), e)
      }
    }
  }
  ...
  private def copyData(oldTableName: String, newTableName: String, ks: String = null): Unit = {
	...
	batchStr.append("\n")
      ...
      case e: Exception => {
        throw CreateDMLException("Unable to copy the table " + oldTableName + ":" + e.getMessage(), e)
      }
    }
  }
