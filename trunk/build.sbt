...
lazy val InputOutputAdapterBase = project.in(file("InputOutputAdapters/InputOutputAdapterBase"))
  .configs(TestConfigs.all: _*)
  .settings(TestSettings.settings: _*)
  .dependsOn(Exceptions, DataDelimiters, HeartBeat)
...
lazy val StorageBase = project.in(file("Storage/StorageBase"))
  .configs(TestConfigs.all: _*)
  .settings(TestSettings.settings: _*)
  .dependsOn(Exceptions, KamanjaUtils, KvBase)
...
lazy val GenericMsgCompiler = project.in(file("GenericMsgCompiler"))
  .configs(TestConfigs.all: _*)
  .settings(TestSettings.settings: _*)
  .dependsOn(Metadata, MetadataBootstrap, Exceptions)
...
lazy val MetadataAPI = project.in(file("MetadataAPI"))
  .configs(TestConfigs.all: _*)
  .settings(TestSettings.settings: _*)
  .dependsOn(StorageManager,Metadata,MessageDef,PmmlCompiler,Serialize,ZooKeeperClient,ZooKeeperListener,OutputMsgDef,Exceptions, SecurityAdapterBase, KamanjaUtils, HeartBeat, KamanjaBase, JpmmlFactoryOfModelInstanceFactory, SimpleApacheShiroAdapter % "test")
...
lazy val InterfacesSamples = project.in(file("SampleApplication/InterfacesSamples"))
  .configs(TestConfigs.all: _*)
  .settings(TestSettings.settings: _*)
  .dependsOn(Metadata, KamanjaBase, MetadataBootstrap, MetadataAPI, StorageBase, Exceptions)
...
lazy val MigrateTo_V_1_3 = project.in(file("Utils/Migrate/DestinationVersion/MigrateTo_V_1_3"))
  .configs(TestConfigs.all: _*)
  .settings(TestSettings.settings: _*)
