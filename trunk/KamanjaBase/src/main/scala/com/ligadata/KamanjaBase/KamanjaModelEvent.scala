...
class KamanjaModelEvent(var transactionId: Long, other: KamanjaModelEvent) extends BaseMsg {
    ...
    var nativeKeyMap  =  scala.collection.mutable.Map[String, String](("modelid", "modelId"), ("elapsedtimeinms", "ElapsedTimeInMs"), ("isresultproduced", "isResultProduced"), ("producedmessages", "ProducedMessages"), ("error", "Error"))
  ...
  private def populateXml(xmlData:XmlData) : Unit = {
    ...
    override def Deserialize(dis: DataInputStream, mdResolver: MdBaseResolveInfo, loader: java.lang.ClassLoader, savedDataVersion: String): Unit = {
	  try {
        ...
        if (prevVer < currentVer) {
                val prevVerObj = new com.ligadata.KamanjaBase.KamanjaModelEvent() 
                prevVerObj.Deserialize(dis, mdResolver, loader, savedDataVersion)   
               	modelid = prevVerObj.modelid;
	elapsedtimeinms = prevVerObj.elapsedtimeinms;
	isresultproduced = prevVerObj.isresultproduced;
	producedmessages = prevVerObj.producedmessages;
	...
	transactionId = com.ligadata.BaseTypes.LongImpl.DeserializeFromDataInputStream(dis);
     ...
     def ConvertPrevToNewVerObj(oldObj : com.ligadata.KamanjaBase.KamanjaModelEvent) : Unit = {    
         if( oldObj != null){
           		modelid = oldObj.modelid;
		elapsedtimeinms = oldObj.elapsedtimeinms;
		isresultproduced = oldObj.isresultproduced;
		producedmessages = oldObj.producedmessages
		error = oldObj.error;
	 ...
	 def withelapsedtimeinms(value: Int) : KamanjaModelEvent = {
