...
object DemoProducerSimpleStats {
...
class DemoCustPartitioner(props: VerifiableProperties) extends Partitioner {
  ...
  def partition(key: Any, a_numPartitions: Int): Int = {
    ...
    try {
      if (key.isInstanceOf[Array[Byte]]) {
        val hashCode = DemoCustPartitioner.keyHasher.hashKey(key.asInstanceOf[Array[Byte]]).abs
 ...
 */
  ...
  * Print stats
  ...
  private def statsPrintFn: Unit = {
    ...
    println("Stats @" + CurTime + "\n\t" + DemoProducerSimpleStats.getDispString("\n\t") + "\n")
  }
  ...
  * Just a local debug method
  ...
  def main(args: Array[String]): Unit = {
    ...
    props.put("partitioner.class", "com.ligadata.tools.DemoCustPartitioner");
