...
class KamanjaData {
  ...
  def appendWithCheck(collection: KamanjaData): Unit = {
    if (key.sameElements(collection.key)) {
      try {
        ...
        case e: Exception => {
          logger.debug("Failed in appendWithCheck", e)
          throw e
        }
      }
    } else {
  ...
  def appendNoDupCheck(collection: KamanjaData): Unit = {
    if (key.sameElements(collection.key)) {
      try {
        ...
        case e: Exception => {
          logger.debug("Failed in appendNoDupCheck", e)
          throw e
        }
      }
    } else {
  ...
  def SerializeData: Array[Byte] = {
    ...
    try {
      ...
      case e: Exception => {
        logger.debug("Failed to SerializeData", e)
        dos.close
        bos.close
        throw e
      }
  ...
  def DeserializeData(bytearray: Array[Byte], mdResolver: MdBaseResolveInfo, loader: java.lang.ClassLoader): Unit = {
    ...
    try {
      ...
      case e: Exception => {
        logger.debug("Failed to DeserializeData", e)
        dis.close
        throw e
      }
    }
