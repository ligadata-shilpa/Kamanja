...
case class KamanjaDataKey_V_1_1_X(T: String, K: List[String], D: List[Int], V: Int)

object KamanjaData_V_1_1_X {
...
*/
    ...
    val key = KamanjaDataKey_V_1_1_X(typName.toLowerCase, partitionkey.map(k => k.toLowerCase), List(StartDateRange, EndDateRange), Version)
    val json =
      ("T" -> key.T) ~
        ("K" -> key.K) ~
        ("D" -> key.D) ~
...
class KamanjaData_V_1_1_X {
  private val ver = KamanjaData_V_1_1_X.Version // Version
  ...
  private var data = ArrayBuffer[MessageContainerBase_V_1_1_X]() // Messages/Containers for this key & with in this date range. 
  ...
  // Getting All Data
  def GetAllData: Array[MessageContainerBase_V_1_1_X] = data.toArray

  // Getting All Data
  def DataSize: Int = data.size

  // Adding New Message or Container
  def AddMessageContainerBase(baseCntMsg: MessageContainerBase_V_1_1_X, checkForSameObj: Boolean, moveToEnd: Boolean): Unit = {
    if (checkForSameObj) {
  ...
  def GetMessageContainerBase(primaryKey: Array[String], newCopyIfFound: Boolean): MessageContainerBase_V_1_1_X = {
  ...
  def appendWithCheck(collection: KamanjaData_V_1_1_X): Unit = {
  ...
  def appendNoDupCheck(collection: KamanjaData_V_1_1_X): Unit = {
    if (key.sameElements(collection.key)) {
      try {
  ...
  def SerializeKey: String = return KamanjaData_V_1_1_X.PrepareKey(typName, key.toList, StartDateRange, EndDateRange)
  ...
  def DeserializeData(bytearray: Array[Byte], mdResolver: MdBaseResolveInfo_V_1_1_X, loader: java.lang.ClassLoader): Unit = {
