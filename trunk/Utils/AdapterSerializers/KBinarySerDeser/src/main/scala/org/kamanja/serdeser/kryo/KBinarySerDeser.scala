...
/**
  ...
  *
  */
class KBinarySerDeser(val mgr : MdMgr
    ...
    def serialize(v : ContainerInterface) : Array[Byte] = {
        ...
        val container : ContainerTypeDef = mgr.ActiveType(containerTypeName).asInstanceOf[ContainerTypeDef]
    ...
    def deserialize(dis : DataInputStream) : ContainerInterface = {
        ...
        val ci : ContainerInterface = objResolver.getInstance(classLoader, containerTypeName)
        if (ci == null) {
        ...
        val containerBaseType : BaseTypeDef = mgr.ActiveType(containerTypeName)
