...
class UpdateModelInPlace extends FunSpec with LocalTestFixtures with BeforeAndAfter with BeforeAndAfterAll with GivenWhenThen {
  var res: String = null;
  ...
  describe("Read all Metadata objects") {
    ...
    it("Read Model Objects") {
	  ...
	  logger.info("objType => " + objType + ",typeName => " + typeName)
          objType match {
            case "modeldef" => {
	      var mdStrBefore:String = new String(v.asInstanceOf[Array[Byte]])
	      ...
	      val n = md1.Name
	      val ns = md1.NameSpace
	      val version = md1.Version.toString
	      MetadataAPIImpl.SaveObject(md1, MdMgr.GetMdMgr)
              res = MetadataAPIImpl.GetModelDef(ns, n, "XML", version, None)
	      logger.info("response => " + res)
              res should include regex ("\"Status Code\" : 0")
	    }
            case _ => {
              logger.debug("Read Model Objects: Ignoring objectType " + objType)
            }
          }
        }
      })
